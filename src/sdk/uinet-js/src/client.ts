...
export interface UINetClientOptions {
...
export class UINetClient {
  ...
  constructor(options: UINetClientOptions = {}) {
    this.timeout = options.timeout ?? 30_000;
    this._explicitTransport = options.transport;

    // The C# side calls window.__uinet_callback(response) to deliver results.
    (window as any).__uinet_callback = (response: BridgeResponse) => {
      this.handleResponse(response);
    };

    // The C# side calls window.__uinet_onevent(event) to push events.
    (window as any).__uinet_onevent = (event: BridgeEvent) => {
      this.handleEvent(event);
    };

    // The C# side calls window.__uinet_invoke(request) for reverse RPC.
    (window as any).__uinet_invoke = (request: ReverseRequest) => {
   ...
   * with the app, the C# host has had time to inject `window.__uinet_native`.
   */
   ...
   *
   * @example
   * const { content } = await uinet.invoke<{ content: string }>('filesystem', 'readText', { path: '/tmp/f.txt' });
   */
  invoke<T = unknown>(module: string, method: string, payload?: unknown): Promise<T> {
    return new Promise<T>((resolve, reject) => {
      ...
      const timer = setTimeout(() => {
        ...
        reject(new Error(`[uinet] Timeout after ${this.timeout}ms for ${module}.${method}`));
      }, this.timeout);
  ...
  private handleEvent(event: BridgeEvent): void {
    ...
    for (const handler of handlers) {
      try {
        ...
        console.error(`[uinet] Error in event handler for '${event.event}':`, e);
      }
    }
  }
...
const isNativeHost = (): boolean => {
  ...
  return typeof (window as any).__uinet_native !== "undefined";
};
