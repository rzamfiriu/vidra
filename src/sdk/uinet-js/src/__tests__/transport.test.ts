...
describe("CustomSchemeTransport", () => {
  ...
  it("appends a hidden iframe with the uinet:// bridge scheme on send", () => {
    ...
    expect(src.startsWith("uinet://bridge?payload=")).toBe(true);

    const decoded = decodeURIComponent(src.replace("uinet://bridge?payload=", ""));
  ...
  it("uses the uinet://reverse scheme for reverse responses", () => {
    const t = new CustomSchemeTransport();
    t.sendReverse({ id: "r", success: true, data: true });

    const src = document.body.querySelector("iframe")?.getAttribute("src") ?? "";
    expect(src.startsWith("uinet://reverse?payload=")).toBe(true);
  });
});
