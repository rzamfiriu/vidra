...
/// Intercepts <c>uinet://bridge</c> navigation requests from JS and dispatches them.
...
public sealed partial class WebViewBridge : IJsCallbackChannel
{
    ...
    {
        await PushToJsAsync("window.__uinet_native = true");
    }

    private async void OnNavigating(object? sender, WebNavigatingEventArgs e)
    {
        if (e.Url.StartsWith("uinet://reverse", StringComparison.OrdinalIgnoreCase))
        {
            e.Cancel = true;
            var payload = Uri.UnescapeDataString(e.Url.Substring("uinet://reverse?payload=".Length));
            HandleReverseResponse(payload);
            return;
        }

        if (!e.Url.StartsWith("uinet://bridge", StringComparison.OrdinalIgnoreCase))
            return;

        e.Cancel = true;

        var bridgePayload = Uri.UnescapeDataString(e.Url.Substring("uinet://bridge?payload=".Length));
        var response = await _dispatcher.DispatchAsync(bridgePayload);

        await PushToJsAsync($"window.__uinet_callback({response})");
    }
    ...
    public async Task SendEventAsync(BridgeEvent bridgeEvent, CancellationToken ct = default)
    {
        ...
        await PushToJsAsync($"window.__uinet_onevent({json})");
    }
    ...
    public async Task<T> CallJsAsync<T>(string handler, object? payload = null, CancellationToken ct = default)
    {
        ...
        await PushToJsAsync($"window.__uinet_invoke({requestJson})");
    ...
    private void HandleReverseResponse(string responseJson)
    {
        ...
        {
            System.Diagnostics.Debug.WriteLine($"[UINet] Failed to parse reverse response: {ex.Message}");
        }
    }
