...
const SHOULD_RUN_DOTNET = process.env.UINET_E2E_DOTNET === "1";

describe("scaffold integration", () => {
  const projectDir = "my-uinet-test-app";
  const projectName = toPascalCase(projectDir);
  let workspace: string;
  let root: string;

  beforeAll(async () => {
    workspace = await fs.mkdtemp(path.join(os.tmpdir(), "uinet-e2e-"));
    root = path.join(workspace, projectDir);

    const replacements: Replacements = {
      "{{projectName}}": projectName,
      "{{projectNameKebab}}": toKebabCase(projectDir),
      "{{appId}}": "com.uinet.myuinettestapp",
