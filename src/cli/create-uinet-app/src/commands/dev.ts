...
export const devCommand = async (argv: string[]): Promise<void> => {
  ...
  const viteUrl = process.env.UINET_DEV_URL || "http://localhost:5173";
...
class DevSession {
  ...
  private readonly buildConfig = process.env.UINET_BUILD_CONFIG || "Debug";
  ...
  async run(): Promise<void> {
    ...
    console.log();
    console.log(`  ${chalk.bold.cyan("uinet dev")} ${chalk.dim(`v${VERSION}`)}`);
    console.log();
  ...
  private installSignalHandlers(): void {
    process.on("SIGINT", () => {
      console.log("\n\x1b[90m[uinet]\x1b[0m Shutting down...");
      this.shutdown(0);
    });
    process.on("SIGTERM", () => {
      this.shutdown(0);
  ...
  private launchMacosHost(): ChildProcess {
    ...
    const host = spawn(binary, [], {
      cwd: this.project.root,
      stdio: ["ignore", "pipe", "pipe"],
      env: { ...process.env, UINET_DEV_URL: this.viteUrl },
    });
