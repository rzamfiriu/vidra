...
const UINET_REPO_ROOT = path.resolve(CLI_ROOT, "..", "..", "..");
const LOCAL_FEED_DIR = path.join(UINET_REPO_ROOT, "dist", "packages");
const LOCAL_CLI_DIR = CLI_ROOT;
const LOCAL_SDK_DIR = path.join(UINET_REPO_ROOT, "src", "sdk", "uinet-js");
const UINET_VERSION = "0.1.0";
const SDK_VERSION = "0.1.0";

const main = async (): Promise<void> => {
  console.log();
  console.log(chalk.bold("  create-uinet-app"));
  console.log(chalk.dim("  Scaffold a new UINet application\n"));
    ...
    const res = await prompts(
      {
        ...
        initial: `com.uinet.${projectNameKebab.replace(/-/g, "")}`,
        validate: (v: string) =>
  ...
  console.log();
  console.log(chalk.green("  Done! ") + "Your UINet app is ready.\n");
  ...
  if (localFeedExists) {
    ...
    console.log(
      chalk.yellow("  Note: ") +
        ...
        " in the UINet repo, then update NuGet.Config.",
    );
  }
