...
When launching through `vidra dev` or packaging through `vidra build --target macos`, Vidra will try to
...
selection with `VIDRA_MACOS_CODESIGN_KEY`. If no usable identity is available, the app may fall back to
